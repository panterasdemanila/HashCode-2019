...
object AlgorithmMaxOfEachIngredient2019 : SolverAlgorithm {
    ...
    fun heuristic(
        ...
        for (i in slice.startRow..slice.endRow) {
            for (j in slice.startColumn..slice.endColumn) {
                if (pizza[i][j] == ProblemData.SliceContent.MUSHROOM) ++tomatoTakens
                else ++mushroomsTakens
            }
        }

        return abs(mushroomLeft - mushroomsTakens - tomatoLeft + tomatoTakens)
    }
    ...
    override fun algorithm(problemData: ProblemData): OutputData {
        ...
        while (allPosibleSlices.isNotEmpty()) {
            val actualMax = allPosibleSlices.maxBy { heuristic(problemData.pizza, numberMushroom, numberTomatos, it) }
